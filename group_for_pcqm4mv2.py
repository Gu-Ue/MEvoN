...
if __name__ == "__main__":
    ...
    # torch.save(data_groups, "/home/data1/lk/project/mol_tree/graph/pcqm4mv2/groups.pth")
    # import pdb;pdb.set_trace()
    data_groups = torch.load("/home/data1/lk/project/mol_tree/graph/pcqm4mv2/groups.pth")

    # 读取 CSV 文件，假设 SMILES 在 'smiles' 列中
    # csv_file_path = "/home/data1/lk/project/mol_tree/graph/pcqm4mv2/pcqm4mv2.csv"
    # df = pd.read_csv(csv_file_path)
    # smiles_list = df['smiles'].tolist()[:50000]  # 限制分子数量

    # 创建 ArgumentParser 对象
    parser = argparse.ArgumentParser(description="Process the group number.")
    
    # 添加命令行参数，type指定参数类型，help指定帮助信息
    parser.add_argument("--runtime", type=int, required=True, help="The group number")
    ...
    import torch 
    data_groups = torch.load("/home/data1/lk/project/mol_tree/groups.pth")
    ...
    for g_num_idx in range(1,51):
        ...
        # import pickle
        # # 保存数据到 .npz 文件
        # save_path = f"/home/data1/lk/project/mol_tree/graph/pcqm4mv2/cluster_results_{group_number}_{runtime}"
        # np.savez( save_path, all_labels=pickle.dumps(all_labels), all_mols=pickle.dumps(all_mols),  all_cluster_centers=pickle.dumps(all_cluster_centers))
        # import pdb;pdb.set_trace()
        # '''TEST END'''
        ...
        # 文件保存路径
        save_path = f"/home/data1/lk/project/mol_tree/graph/pcqm4mv2/cluster_results_{group_number}"
        import pickle
        # 保存数据到 .npz 文件
