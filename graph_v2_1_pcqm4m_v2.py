...
def read_cluster(N):
    ...
    npz_file = f"/home/data1/lk/project/mol_tree/graph/pcqm4mv2/cluster_results_{N}.npz"
...
def construct_graph(smiles_list: List[str], construct_graph_method: list, threshold: float) -> [Graph, defaultdict]:
    ...
    for atom_count in tqdm(sorted(atom_groups.keys())):
        # 临时命令 START
        if atom_count<=10 or atom_count>=26:
            continue
        # END
        ...
        KK = 1
        # if group_list[atom_count]['cluster_group'] else 2
        ...
        for _, cluster_index in enumerate(tqdm(sorted(group_list[atom_count]['sample_group']))):
            ...
            for molecule_b in next_group:
                ...
                if similarities_pair:
                    ...
                    with open('/home/data1/lk/project/mol_tree/graph/pcqm4mv2/bad_smiles.txt', 'a') as file:
                        file.write(Chem.MolToSmiles(molecule_b) + '\n')  # 每个 SMILES 换行写入
                        
        save_path = f'/home/data1/lk/project/mol_tree/graph/pcqm4mv2/temp_atom_count_{atom_count}_evolution_graph_{len(smiles_list)}_{construct_graph_method}_{threshold}_v2.json'  # Specify the path where you want to save the graph
