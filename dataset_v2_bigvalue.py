...
class MoleculeACDataset_v2(Dataset):
    ...
    def normalize(self):
        # 提取所有的self.label_name字段的值
        label_values = [value[self.label_name] for value in self.labels_dict.values()]
        
        # 计算均值和标准差
        self.mean = np.mean(label_values)
        self.std = np.std(label_values)

        # 对字典中的值进行归一化
        for key, value in self.labels_dict.items():
            value[self.label_name] = (value[self.label_name] - self.mean) / self.std
    ...
    def construct_dataset(self):
        ...
        # self.labels_dict = {key: {**value, self.label_name: -1 if value[self.label_name] == 0.0 else value[self.label_name]} for key, value in self.labels_dict.items()}
...
# *27.211407953
            ...
            if mol_2_label[self.label_name]!=0:
    ...
    def __getitem__(self, idx):
        ...
        # target = torch.tensor([  ratio, label_list[0], label_list[1]], dtype=torch.float64)
