...
log_file = os.path.join(log_dir, f'only_schnet_qm9_{label_name}_training_log_{start_time}.log')
...
if label_name != 'Cv':
    optimizer = optim.AdamW(model.parameters(), lr=0.0005, weight_decay=1e-4) # 0.0005
else:
    optimizer = optim.AdamW(model.parameters(), lr=0.00045, weight_decay=1e-4) # 0.0005
...
def predict(model, dataloader, mode=None):
    ...
    with torch.no_grad():
        for graph1, graph2, target, paths, paths_labels, paths_labels_masks in dataloader:
            ...
            output = model.forward_origin(graph1)

            # 分别计算两个回归项的损失
            loss1 = criterion(output.squeeze().float(), target[:, 1].float())
...
def train(model, train_dataloader, val_dataloader, optimizer, criterion, epochs=1000):
    ...
    for epoch in tqdm(range(epochs),desc=f'for {label_name}:'):
        ...
        if val_loss < best_val_loss:
            ...
            pth_file = os.path.join(pth_dir, f'schnet_{label_name}_best_model_epoch_{start_time}.pth')
