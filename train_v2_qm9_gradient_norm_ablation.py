...
from model_ablation import GNNModel
# from dataset_v2 import MoleculeACDataset_v2
# from dataset_v2_test import MoleculeACDataset_v2
from dataset_v2_test_Ha2eV_ablation import MoleculeACDataset_v2
...
log_file = os.path.join(log_dir, f'qm9_ablation_{label_name}_training_log_{start_time}.log')
...
files_to_log = [
    '/home/data1/lk/project/mol_tree/train_v2_qm9_gradient_norm_ablation.py',
    '/home/data1/lk/project/mol_tree/tools.py',
    '/home/data1/lk/project/mol_tree/model_ablation.py',
    # '/home/data1/lk/project/mol_tree/dataset_v2.py'
    "/home/data1/lk/project/mol_tree/dataset_v2_test_Ha2eV_ablation.py"
]
...
# model_checkpoint = "/home/data1/lk/project/mol_tree/checkpoint/gnn_model_epoch_20250106_075021_epoch_125.pth"
...
def predict(model, dataloader, weights, mode=None):
    model.eval()
    total_loss = 0
    total_loss1 = 0
    total_loss2 = 0
...
def infer(model, dataloader, weights, mode=None):
    model.eval()
    total_loss = 0
    total_loss1 = 0
    total_loss2 = 0
...
def train(model, train_dataloader, val_dataloader, optimizer, criterion, epochs=1000):
    ...
    for epoch in tqdm(range(epochs),desc=f'for {label_name}:'):
        ...
        for iter, (graph1, graph2, target, paths, paths_labels, paths_labels_masks) in enumerate(train_dataloader):
            ...
            # Step 3: Compute gradients before backward pass (without actually updating model)
            loss1.backward(retain_graph=True)  # Retain graph to compute gradients for both losses
            ...
            # # List the specific parameters to track
            # params_to_track_1 = [
            #     model.fc1_1.weight,
            #     model.fc2_1.weight,
            #     model.fc3_1.weight,
            ...
            # for param in params_to_track_1:
            #     if param.grad is not None:
            #         grad_norm1 += torch.norm(param.grad).item()

            # for param in params_to_track_2:
            #     if param.grad is not None:
            #         grad_norm2 += torch.norm(param.grad).item()
                    

            # # Step 5: Compute the target gradient norm (mean of both gradient norms)
            # grad_target = (grad_norm1 + grad_norm2) / 2

            # # Step 6: Compute weights based on gradient norms (inverse of the gradient norms)
            # weight1 = grad_target / grad_norm1
            # weight2 = grad_target / grad_norm2

            # Step 7: Adjust the total loss with dynamic weights
            ...
            total_loss1 += loss1.item()
            total_loss2 += loss2.item()
            # log(f'Loss1 weight:{weight1}\t Loss2 weight:{weight2}')
        ...
        # Validate
        val_loss, val_loss1, val_loss2 = predict(model, val_dataloader, [weight1,weight2])
        ...
        if val_loss1 < best_val_loss:
            ...
            test_loss, test_loss1, test_loss2 = infer(model, test_dataloader, [weight1,weight2],  mode='test')
