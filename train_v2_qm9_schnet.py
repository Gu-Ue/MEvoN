...
# 初始化模型和优化器
# modify
device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu") # cuda:0 cuda:1 cuda:2
is_mu = label_name =='mu'
is_r2 = label_name == 'r2'
model = GNNModel(device, is_mu=is_mu, is_r2=is_r2).to(device)  # Move model to GPU
...
# 学习率调度器
if label_name in ['zpve', 'Cv', 'alpha', 'r2', 'mu']:
    scheduler = ReduceLROnPlateau(optimizer, mode="min", factor=0.8, patience=5, min_lr=1e-9)
else:
...
def load_checkpoint(model, optimizer, scheduler, checkpoint_dir, start_time):
    ...
    if os.path.exists(pth_file):
        ...
        start_epoch = checkpoint['epoch'] + 1
        best_val_loss = checkpoint['best_val_loss']
        log(f"Loaded checkpoint from {pth_file}, epoch {checkpoint['epoch']}, best_val_loss {best_val_loss:.6f}")
        return start_epoch, best_val_loss
    else:
        log("No checkpoint found, starting from scratch.")
        return 0, float('inf')'''
